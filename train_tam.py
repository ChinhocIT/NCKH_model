...
class Config:
    ...
    # =========================================================
    # OPTIMIZED FOR P40 24GB / V100 32GB GPU
    # =========================================================
    BATCH_SIZE = 6        # P40 24GB (FP32) or V100 32GB (FP16)
    GRAD_ACCUM_STEPS = 3  # Effective batch = 6 * 3 = 18
    NUM_EPOCHS = 30
    ...
    # =========================================================
    # GPU OPTIMIZATION (P40/V100/A100)
    # =========================================================
    NUM_WORKERS = 4       # More workers for faster data loading
    PIN_MEMORY = True     # Pin memory for faster GPU transfer
    # NOTE: Set False for P40 (no Tensor Cores), True for V100/A100
    MIXED_PRECISION = False  # P40: FP32 faster | V100/A100: set True for FP16
...
class QwenViTEncoder:
    ...
    def __init__(self, model_name="Qwen/Qwen2-VL-2B-Instruct", device="cuda"):
        ...
        # Load with FP32 for P40 (no Tensor Cores, FP32 is faster)
        # NOTE: For V100/A100, change to torch.float16 for speed
        self.full_model = AutoModelForVision2Seq.from_pretrained(
            model_name,
            torch_dtype=torch.float32,  # P40: FP32 | V100/A100: use torch.float16
            device_map="auto",
